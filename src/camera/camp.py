...
class OptimizableCamera(Camera, nn.Module):
    def __init__(self, *args, **kwargs):
        ...
        # Parameters:
        self.residual_quat = nn.Parameter(torch.tensor([1,0,0,0], dtype=torch.float32, device='cpu'), requires_grad=True)
        self.residual_trans = nn.Parameter(torch.zeros(3, dtype=torch.float32, device='cpu'), requires_grad=True)
